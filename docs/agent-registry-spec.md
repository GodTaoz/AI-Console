...
- 持久 watcher 优先通过 Codex `app-server thread/list` 读取最近会话的 `id`、名称和更新时间，失败时回退会话索引的 `id`、`thread_name`、`updated_at`；来源均为 `discovered`，不会读取 `history.jsonl`、prompt 或完整对话。
...
- systemd unit 显式增加 `--codex-source auto --codex-limit 20 --all` 和 Hermes 发现参数；普通 `bootstrap-local` 默认保持旧索引单会话发现，避免意外导入大量历史记录。
...
- “任务与收件箱”明确区分两种动作：`发送并执行` 通过 Runtime Bridge 在源会话启动新 turn；`仅保存到收件箱` 只写入独立 inbox，不与源历史混合，也不会自动唤醒智能体。
