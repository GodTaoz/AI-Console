...
| `/api/v1/agent-sessions...` | GET / PUT / POST / PATCH | 智能体会话注册、心跳、列表、详情和 entry |
| `/api/v1/agent-tree` | GET | 智能体父子会话树 |
| `/api/v1/agent-discovery` | GET | 本机发现器状态、最后扫描时间和发现数量 |
| `/api/v1/agent-discovery/{source_id}` | PUT | watcher 上报一次类型化扫描结果 |
| `/api/v1/agent-sessions/{session_id}/observation` | PUT | 上报 tmux/process/cron carrier 只读对账结果 |
| `/api/v1/agent-sessions/{session_id}/inspect` | GET | 返回安全会话摘要并记录审计 |
| `/api/v1/agent-sessions/{session_id}/resume-hint` | GET | 返回白名单恢复提示，不执行命令 |
| `/api/v1/agent-audit` | GET | 查询最近智能体操作审计 |
| `/api/v1/agent-sessions/{session_id}/messages` | GET / POST | 查询或发送本机会话 inbox 消息 |
| `/api/v1/agent-messages/{message_id}/ack` | POST | 确认一条 inbox 消息 |
| `/api/v1/agent-sessions/{session_id}/history` | GET | 通过宿主 Bridge 按需读取脱敏后的真实历史 |
| `/api/v1/agent-sessions/{session_id}/turns` | POST | 向原 Hermes/Codex 会话提交新 turn |
| `/api/v1/agent-sessions/{session_id}/models` | GET | 获取该运行时当前可用模型列表 |
| `/api/v1/agent-sessions/{session_id}/name` | PATCH | 同步修改源会话和注册记录名称 |
| `/api/v1/agent-turns/{run_id}/events` | GET | SSE 流式回复、运行阶段、工具摘要和审批事件 |
| `/api/v1/agent-sessions/{session_id}/archive` | POST | 本地软归档会话 |
| `/api/v1/agent-sessions/{session_id}/delete-source` | POST | 强确认后删除源会话 |
...
| `/agents` | 智能体会话 | 默认会话工作区：可折叠/拖动会话栏、真实历史、安全 Markdown、阶段状态、模型选择、附件、流式续聊、重命名、审批和生命周期操作 |
| `/agents/status` | 智能体状态 | 活动、空闲、等待、失败/失联数量，发现器健康、Runtime Bridge 状态和最近会话表格 |
...
- Topbar 只承载导航开关、语言与主题控制，不重复显示页面标题或说明。
- 内容区不重复显示页面标题和说明；页面身份由侧边栏导航、模块页签和区块标题表达，采集状态、更新时间与刷新动作保留在内容顶部。
- 监控页面使用文档自然滚动，数据表默认每页 10 条且数据不超过 10 条时隐藏分页；仅会话、日志等连续上下文使用组件内部滚动。
- 智能体会话列表优先展示活动会话，并在同一浏览器标签页中记住最后打开的会话以及“会话 / 状态监控”子路由。
- 智能体任务区必须区分“通过 Runtime Bridge 发送并执行”和“仅保存到 SQLite 收件箱”，不得把本地 inbox 描述成会自动唤醒运行时。
...
智能体会话注册与运行观测模块使用独立规格：[`agent-registry-spec.md`](agent-registry-spec.md)。Phase 1 按该规格实现，保持为单机运行资源域，不扩张为完整控制平面。
